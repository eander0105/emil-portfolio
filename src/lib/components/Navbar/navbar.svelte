<script lang="ts">
	import PhoneNav from './navbar-phone.svelte';

	let { children } = $props();
</script>

<header class="sticky top-0 z-50 bg-background">
	<nav class="mx-auto max-w-[1024px] flex flex-row justify-between border-b px-4 py-6 md:py-4 md:pb-4">
		...
		<!-- Desktop links -->
		<div class="left-auto hidden h-auto w-auto flex-row p-0 md:flex">
			{@render children?.()}
		</div>

		<!-- Mobile links/menu -->
		<div class="flex h-auto w-auto items-center justify-end md:hidden">
			<PhoneNav>
				{@render children?.()}
			</PhoneNav>
		</div>
